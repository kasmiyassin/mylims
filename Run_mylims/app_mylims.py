...
def global_search(search_term: str):
    ...
    queries: Dict[str, Tuple[str, Tuple[Any, ...]]] = {
        ...
        "reagents": (
            f'SELECT reagent_id, reagent_complete_name, lot FROM "lims"."reagents" WHERE "reagent_search_vector" @@ {ts_query_func} OR "reagent_id" ILIKE %s OR "reagent_complete_name" ILIKE %s OR "lot" ILIKE %s LIMIT 5',
...
def get_table_data(schema: str, table: str):
    ...
    try:
        ...
        def parse_datetime_filter(dt_str):
            ...
            try:
                # Handle YYYY-MM-DDTHH:MM format from HTML input
...
def update_record(schema: str, table: str):
    ...
    try:
        ...
        for key, val in data.items():
            ...
            # CRITICAL FIX FOR SINGLE UPDATE: If the field is a standard string type and is empty, skip it.
            if isinstance(val, str) and val == '':
                # Only proceed if we are explicitly clearing an attachment link, otherwise ignore empty fields
    ...
    try:
        ...
        if (actual_schema.lower() == 'lab' and actual_table.lower() == 'experiments') or \
           (actual_schema.lower() == 'lims' and actual_table.lower() == 'projects'):
            ...
            # Since request.get_json() returns the list of records, we need to check if the *first*
            # element is malformed, indicating a deeper issue upstream that forces a bad format.
                ...
                if len(first_record) == 1 and isinstance(list(first_record.keys())[0], str) and ';' in list(first_record.keys())[0]:
                    ...
                    for record in records:
                        if len(record) == 1 and list(record.keys())[0] == malformed_key:
                            ...
                            # If a record isn't the expected malformed format, it's likely already correct
...
def batch_update(schema: str, table: str):
    ...
    try:
        ...
        # --- CRITICAL FIX 2: Check for malformed single-key payload in batch_update ---
                ...
                for record in records_to_update:
                    if len(record) == 1 and list(record.keys())[0] == malformed_key:
                        ...
                        if len(headers) == len(values):
                            ...
                            print(f"WARNING: Skipping batch update record due to header/value mismatch after parsing: {record}")
                    else:
                        new_records.append(record) 
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            for record_data in records_to_update:
                ...
                for key, val in update_fields.items():
                    ...
                    # CRITICAL FIX 3: If the value is an empty string, SKIP it, 
                    # as this signals "no change" in a bulk modify operation.
                    if isinstance(val, str) and val == '':
                        # Special case: only process the empty string if the column is explicitly 'attachment_link'
                        if key != 'attachment_link':
                            continue
                        # If it is 'attachment_link' and empty, set to None below.
