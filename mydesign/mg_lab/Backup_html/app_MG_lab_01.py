...
def get_pk_column(schema, table):
    # This logic remains as it correctly defines the primary key for specific tables.
    special_cases = {
        ('Lab', 'Fish'): 'sample_id', ('Lab', 'Tissue'): 'sample_id',
        ('Lab', 'DNA'): 'sample_id', ('Lab', 'RNA'): 'sample_id',
        ('Lab', 'Sediments'): 'sample_id', ('Lab', 'Water'): 'sample_id',
        ('Lims', 'Projects'): 'project_id',
        ('Lab', 'Experiments'): 'Experiments_Nr',
    }
    # Use schema and table name directly without case change for matching
    return special_cases.get((schema, table), 'nr')
...
# --- *** CORRECTED GENERIC TABLE ENDPOINTS *** ---

@app.route('/api/table/<string:schema>/<string:table>', methods=['GET'])
def get_table_data(schema, table):
    """Generic function to fetch all data from a table, using exact case for table names."""
    # FIX: Removed .capitalize() from table to respect case-sensitivity (e.g., 'SamplesType')
...
def create_record(schema, table):
    if request.is_json: data = request.get_json(); files = {}
    else: data = request.form.to_dict(); files = request.files
    if not data and not files: return jsonify({"error": "No data provided"}), 400
    if 'attachment' in files: data['attachment'] = psycopg2.Binary(files['attachment'].read())
    filtered_data = {k: v for k, v in data.items() if v is not None and v != ''}
    columns = filtered_data.keys(); values = list(filtered_data.values())
    column_names = ', '.join([f'"{col}"' for col in columns]); value_placeholders = ', '.join(['%s'] * len(values))
    # FIX: Removed .capitalize() from table
    query = f'INSERT INTO "{schema.capitalize()}"."{table}" ({column_names}) VALUES ({value_placeholders}) RETURNING *;'
    conn = get_db_connection()
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(query, values); new_record = cur.fetchone(); conn.commit()
...
def update_record(schema, table, pk_value):
    pk_column = get_pk_column(schema, table); data = request.get_json()
    if not data: return jsonify({"error": "No data provided"}), 400
    set_clause = ', '.join([f'"{key}" = %s' for key in data.keys()]); values = list(data.values())
    try: values.append(int(pk_value) if pk_column == 'nr' else pk_value)
    except ValueError: return jsonify({"error": f"Invalid ID format: {pk_value}"}), 400
    # FIX: Removed .capitalize() from table
    query = f'UPDATE "{schema.capitalize()}"."{table}" SET {set_clause} WHERE "{pk_column}" = %s RETURNING *;'
    conn = get_db_connection()
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(query, values); updated_record = cur.fetchone(); conn.commit()
...
def delete_record(schema, table, pk_value):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(query, (int(pk_value) if pk_column == 'nr' else pk_value,)); conn.commit()
            if cur.rowcount == 0: return jsonify({"error": "Record not found"}), 404
        return "", 204
    except Exception as e: conn.rollback(); return jsonify({"error": str(e)}), 500
    finally: conn.close()
...
def batch_upload(schema, table):
    ...
    if not records or not isinstance(records, list): return jsonify({"error": "Invalid data format."}), 400
    conn = get_db_connection()
    try:
        with conn.cursor() as cur:
            columns = [col for col in records[0].keys()]; column_names = ', '.join([f'"{col}"' for col in columns])
            # FIX: Removed .capitalize() from table
            query_template = f"INSERT INTO \"{schema.capitalize()}\".\"{table}\" ({column_names}) VALUES %s"
            data_tuples = [tuple(record.get(col) for col in columns) for record in records]
            psycopg2.extras.execute_values(cur, query_template, data_tuples); conn.commit()
...
# =================================================================
@app.route('/')
def root(): return redirect(url_for('serve_static', filename='login.html'))
