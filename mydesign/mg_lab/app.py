...
def get_table_data(schema: str, table: str):
    try:
        ...
        # Extract filter_project_id specifically, if present in query parameters
        filter_project_id_value = request.args.get('filter_project_id')
        
        base_query = f'SELECT * FROM "{actual_schema}"."{actual_table}"'

        # --- SPECIAL HANDLING FOR lab.experiments WHEN FILTERED BY project_id ---
        if actual_schema.lower() == 'lab' and actual_table.lower() == 'experiments' and filter_project_id_value:
            # Modify the base query to perform a JOIN with experiments_projects
            base_query = f"""
                SELECT E.*
                FROM "{actual_schema}"."{actual_table}" AS E
                JOIN "lab"."experiments_projects" AS EP ON E.experiment_id = EP.experiment_id AND E.experiment_date = EP.experiment_date
            """
            ...
            # The filter_project_id will be handled by this special block, so it won't be processed by generic filters below.
        ...
        # Iterate through all query arguments to apply filters, skipping filter_project_id if already handled for experiments
        for key, value in request.args.items():
            if not value:
                continue
            
            # Skip filter_project_id if it was just handled specifically for lab.experiments
            if key == 'filter_project_id' and actual_schema.lower() == 'lab' and actual_table.lower() == 'experiments':
                continue
            ...
            # Standard list of filterable columns
            filterable_columns = [
                'sample_id', 'status_id', 'sop_id', 'person_id', 'project_id', # project_id is kept here for other tables that directly have it
                'room_id', 'vessel_id', 'region_id', 'ecosystem_id', 'category_id', 'sample_type_id', 'gene_id', 'taxon_id', 'species_id', 'unit_id', # Reference IDs
                'contact_id', 'customer_id', 'workflow_id', 'permit_id', 'primer_id', 'step_number', 'equipment_id', 'supplier_id', 'item_id', 'fi_order_nr', 'reagent_id', 'publication_type_id', 'publication_id', # Lims IDs
                ...
                'reception_date', 'experiment_date', 'dissection_date', 'extraction_date', 'measurement_date', 'pcr_date', 'run_date', 'prep_date', 'sequencing_date', 'order_date', 'expire_date', 'date_publication', 'date_submission', 'valid_from', 'valid_to', 'link_date', 'move_date', 'action_timestamp', 'created_at', 'date_realise',
            ]
