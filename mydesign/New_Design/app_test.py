...
def get_table_data(schema: str, table: str):
    try:
        ...
        filter_sampling_date_value = request.args.get('filter_sampling_date') # For samples and related tables
        ...
        # --- End Special Handling for project_id/sample_id/experiment_id ---
        ...
        for key, value in request.args.items():
            ...
            if (key == 'filter_project_id' and filter_project_id_value) or \
               ...
               (key == 'filter_sampling_date' and filter_sampling_date_value):
                continue
            ...
            filterable_columns = [
                ...
                'reception_date', 'experiment_date', 'dissection_date', 'extraction_date', 'measurement_date', 'pcr_date', 'run_date', 'prep_date', 'sequencing_date', 'order_date', 'expire_date', 'date_publication', 'date_submission', 'valid_from', 'valid_to', 'link_date', 'move_date', 'action_timestamp', 'created_at', 'date_realise', 'sampling_date' # Added sampling_date here
            ]
            ...
            for record_data in records_to_update:
                ...
                for key, val in record_data.items():
                    if key in pk_columns:
                        ...
                        if 'date' in key.lower() and pk_values_from_request is not None and key in pk_values_from_request and isinstance(pk_values_from_request[key], str):
                            try:
                                val = datetime.strptime(pk_values_from_request[key], '%Y-%m-%d').date()
                            except ValueError:
                                pass # Keep as string if not a valid date format
