...
-- Function to populate sample_creation_date for all child sample tables (dna, rna, fish, tissue, sediments, water, pcr, otoliths) from lab.root_samples.
CREATE OR REPLACE FUNCTION "lab".populate_sample_creation_date()
RETURNS TRIGGER AS $$
DECLARE
    parent_date date;
BEGIN
...
-- Function to populate experiment_date for child tables that link to lab.experiments.
CREATE OR REPLACE FUNCTION "lab".populate_experiment_date()
RETURNS TRIGGER AS $$
DECLARE
    parent_date date;
BEGIN
...
-- Function to populate sampling_date for child tables that link to lab.sampling.
CREATE OR REPLACE FUNCTION "lab".populate_sampling_date()
RETURNS TRIGGER AS $$
DECLARE
    parent_date date;
BEGIN
...
-- Function to populate reception_date for datasets from lab.root_samples.
CREATE OR REPLACE FUNCTION "lab".populate_dataset_reception_date()
RETURNS TRIGGER AS $$
DECLARE
    parent_date date;
BEGIN
    IF NEW.reception_date IS NULL AND NEW.sample_id IS NOT NULL THEN
        ...
        NEW.reception_date := parent_date;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Function to populate sequencing_date for analysis runs.
CREATE OR REPLACE FUNCTION "bioinformatics".populate_sequencing_date()
RETURNS TRIGGER AS $$
DECLARE
    parent_date date;
BEGIN
    IF NEW.sequencing_date IS NULL AND NEW.sequencing_id IS NOT NULL THEN
        SELECT "experiment_date" -- In sequencing_run, this is the date field
        INTO parent_date
...
-- ======================================================================
-- B. GENERIC PARTITION CREATION FUNCTION
-- ======================================================================

-- This function dynamically creates a yearly partition for any parent table.
CREATE OR REPLACE FUNCTION "lab".create_partition_if_not_exists()
RETURNS TRIGGER AS $$
DECLARE
    ...
    end_date date;
BEGIN
    -- Dynamically determine the parent table and schema from the trigger context.
    partition_schema := TG_TABLE_SCHEMA;
    partition_table := TG_TABLE_NAME;
    partition_column_value := (SELECT NEW.partition_date);

    IF partition_column_value IS NULL THEN
        RAISE EXCEPTION 'Partitioning column value is NULL. Cannot create partition.';
    END IF;

    -- Generate the start and end dates for the new yearly partition.
    start_date := DATE_TRUNC('year', partition_column_value::date);
    end_date := DATE_TRUNC('year', partition_column_value::date) + INTERVAL '1 year';
    
    -- Generate the unique partition name.
    partition_name := partition_table || '_' || TO_CHAR(start_date, 'YYYY');

    -- Create the partition if it does not already exist.
    EXECUTE 'CREATE TABLE IF NOT EXISTS ' || quote_ident(partition_schema) || '.' || quote_ident(partition_name) || ' PARTITION OF ' || quote_ident(partition_schema) || '.' || quote_ident(partition_table) || ' FOR VALUES FROM (''' || start_date || ''') TO (''' || end_date || ''');';
...
-- Triggers for tables partitioned by sample_creation_date
CREATE TRIGGER trg_populate_dna_date
BEFORE INSERT ON "lab"."dna"
FOR EACH ROW EXECUTE FUNCTION "lab".populate_sample_creation_date();

CREATE TRIGGER trg_create_dna_partition
BEFORE INSERT ON "lab"."dna"
FOR EACH ROW EXECUTE FUNCTION "lab".create_partition_if_not_exists();

-- ...and so on for all sample-related tables
...
CREATE TRIGGER trg_populate_root_samples_date
...
-- Triggers for tables partitioned by experiment_date
CREATE TRIGGER trg_populate_experiments_projects_date
BEFORE INSERT ON "lab"."experiments_projects"
FOR EACH ROW EXECUTE FUNCTION "lab".populate_experiment_date();
...
CREATE TRIGGER trg_populate_dissections_date
...
CREATE TRIGGER trg_populate_nanodrop_date
...
CREATE TRIGGER trg_populate_qubit_date
...
CREATE TRIGGER trg_populate_tapestation_date
...
CREATE TRIGGER trg_populate_gelelectrophoresis_date
...
CREATE TRIGGER trg_populate_qpcr_date
...
CREATE TRIGGER trg_populate_library_date
...
CREATE TRIGGER trg_populate_sequencing_run_date
...
-- Triggers for tables partitioned by sampling_date
CREATE TRIGGER trg_populate_fishing_date
BEFORE INSERT ON "lab"."fishing"
FOR EACH ROW EXECUTE FUNCTION "lab".populate_sampling_date();
...
CREATE TRIGGER trg_populate_sampling_abiotic_date
BEFORE INSERT ON "lab"."sampling_abiotic_data"
FOR EACH ROW EXECUTE FUNCTION "lab".populate_sampling_date();
CREATE TRIGGER trg_create_sampling_abiotic_partition
BEFORE INSERT ON "lab"."sampling_abiotic_data"
FOR EACH ROW EXECUTE FUNCTION "lab".create_partition_if_not_exists();


-- Triggers for other partitioned tables
CREATE TRIGGER trg_populate_datasets_date
BEFORE INSERT ON "lab"."datasets"
FOR EACH ROW EXECUTE FUNCTION "lab".populate_dataset_reception_date();
CREATE TRIGGER trg_create_datasets_partition
BEFORE INSERT ON "lab"."datasets"
FOR EACH ROW EXECUTE FUNCTION "lab".create_partition_if_not_exists();

CREATE TRIGGER trg_populate_analysis_runs_sequencing_date
BEFORE INSERT ON "bioinformatics"."analysis_runs"
FOR EACH ROW EXECUTE FUNCTION "bioinformatics".populate_sequencing_date();
CREATE TRIGGER trg_create_analysis_runs_partition
BEFORE INSERT ON "bioinformatics"."analysis_runs"
FOR EACH ROW EXECUTE FUNCTION "lab".create_partition_if_not_exists();

CREATE TRIGGER trg_create_wanderfische_fishing_partition
